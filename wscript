...
VERSION = '0.1.0'
APPNAME = 'ndn-repo-client'
GIT_TAG_PREFIX = 'ndn-repo-client-'
...
def options(opt):
    ...
    optgrp = opt.add_option_group('ndn-repo-client Options')
...
def configure(conf):
    ...
    conf.write_config_header('config.hpp', define_prefix='NDN_REPO_CLIENT_')

def build(bld):
    libndn_repo_client = dict(
        target='ndn-repo-client',
        vnum=VERSION,
        cnum=VERSION,
        source=bld.path.ant_glob('ndn-repo-client/**/*.cpp'),
        use='NDN_CXX BOOST',
        includes='ndn-repo-client .',
        export_includes='ndn-repo-client .',
        install_path='${LIBDIR}')

    if bld.env.enable_shared:
        bld.shlib(name='ndn-repo-client',
                  **libndn_repo_client)

    if bld.env.enable_static:
        bld.stlib(name='ndn-repo-client-static' if bld.env.enable_shared else 'ndn-repo-client',
                  **libndn_repo_client)
    ...
    headers = bld.path.ant_glob('ndn-repo-client/**/*.hpp')
    bld.install_files('${INCLUDEDIR}', headers, relative_trick=True)

    bld.install_files('${INCLUDEDIR}/ndn-repo-client',
                      bld.path.find_resource('config.hpp'))

    bld(features='subst',
        source='libndn-repo-client.pc.in',
        target='libndn-repo-client.pc',
